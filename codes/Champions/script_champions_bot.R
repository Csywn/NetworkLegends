...
b = read.table("champions_bot_name.txt",header=FALSE)
b = data.frame(group=c(0:(length(b[,1])-1)),name=b[,3])
x = rep(0,length(b[,1]))
y = rep(0,length(b[,1]))
b = b[order(b$name),]
...
###Links
wins = read.table("bot_win.txt",header=FALSE)
wins[,1] = wins[,1] - 1
wins[,2] = wins[,2] - 1
...
a$nodes = b
export = toJSON(a)
write(export,"champions_bot.json")
