...
tops = read.table("champions_jungle.txt",header=FALSE)
...
for(i in 1:length(ids))
{
  ...
  a = fromJSON(name)
  top_n = which(a$participants$timeline$lane=="JUNGLE")
  top = a$participants$championId[which(a$participants$timeline$lane=="JUNGLE")]
...
write.table(duo_win,"jungle_win.txt",row.names=FALSE,col.names=FALSE)
