...
for(i in 1:length(ids))
{
  ...
  a = fromJSON(name)
  top_n = which(a$participants$timeline$lane=="TOP")
  top = a$participants$championId[which(a$participants$timeline$lane=="TOP")]
...
write.table(duo_win,"top_win.txt",row.names=FALSE,col.names=FALSE)
