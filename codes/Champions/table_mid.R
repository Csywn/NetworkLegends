...
for(i in 1:max)
{
  ...
  a = fromJSON(name)
  top = c(top,a$participants$championId[which(a$participants$timeline$lane=="MIDDLE"|a$participants$timeline$lane=="MID")])
}
...
for(i in 1:max)
{
  name = sprintf("../matches/match_%.0f.json",ids[i])
  a = fromJSON(name)
  champions = which(a$participants$timeline$lane=="MIDDLE"|a$participants$timeline$lane=="MID")
