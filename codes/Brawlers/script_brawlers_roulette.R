...
a = fromJSON("../Champions/champions_backup.json")

b = read.table("brawlers_groups.txt",header=TRUE)
b = b[1:50,]
b = data.frame(group=c(0:(length(b[,1])-1)))
x = rep(0,length(b[,1]))
y = rep(0,length(b[,1]))
for(i in 0:(length(b[,1])-1))
{
  alpha = i*360/length(b[,1])
  r=radius
...
b= cbind(b,fixed)
colnames(b) = c("group","id","x","y","fixed")
b[,1] = b[,1] + 1
#colnames(b) = c("group","name","id","fixed")
#b=b[which(b$group==tops),]

###Links
wins = read.table("brawlers_win.txt",header=FALSE)
wins[,1] = wins[,1] - 1
wins[,2] = wins[,2] - 1
...
{
  d[i*2-1,] = c(wins[i,1],wins[i,2],(wins[i,3]-wins[i,4])/(wins[1,3]+wins[1,4]))
  d[i*2,] = c(wins[i,2],wins[i,1],(wins[i,4]-wins[i,3])/(wins[1,3]+wins[1,4]))
}
